...
public class ExecuteSearchState extends ApplicationModelState {
	 ...
	 * some cases, this can take 2-3 seconds, especially if the user set no
	 * required effects.
	 */
	...
	/**
	 * Creates a Task object and runs the main graph computation in its own
	 * thread. This is required to keep the UI responsive (i.e. show the log
	 * message).
	 */
	...
	/**
	 * Performs a variety of modeling and graph computations to settle on results
	 * to show the user.
	 */
	...
	private void addPotionToResultSet(final Set<Ingredient> ingredients) {
		...
		final Set<PotionEffect> potionEffects = matchedEffects.stream().map(
				potionEffect -> ModelFactory.getInstance().createPotionEffect(ingredients, potionEffect, this.characterConfig))
	...
	/**
	 * This method updates the UI with the new results. This must be run back on
